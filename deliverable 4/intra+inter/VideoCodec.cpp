...
void VideoCodec::compress(std::string &inputFile, std::string &compressedFile) {
    ...
    int numFrames = calcNFrames(inputFile, inFileData);
    compressedHeaderBuild(headerStr, initial_m, numFrames);
    std::cout << "Tamanho do cabeçalho: " <<headerStr.size() << std::endl;
    encoder.encodeHeader(std::to_string(headerStr.size()));              // encodes the header size at the begining
...
void VideoCodec::decompress(std::string &outputFile, std::string &compressedFile) {
    ...
    unsigned int headerSize = stoi(decoder.decodeHeader(2));
...
void VideoCodec::compressedHeaderBuild(std::string& compressedHeader, int m, int nFrames) {
    ...
    std::string str_estimationBlockSize = " BS" + std::to_string(estimationBlockSize);
    compressedHeader += str_estimationBlockSize;
    std::string str_estimation = " ES" + std::to_string(estToInt(estimation)) + " Z\n";
